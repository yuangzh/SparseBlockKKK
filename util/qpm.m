...
function [x,cc,his] = QPM_time(R,y,n_spikes)
hR = @(x) R*x;
hRt = @(x) R'*x;
[x,cc,his] = PD_CS(hR,hRt,'f',y,-inf,inf,n_spikes,1e-3,300);
...
function [x,cc,his] = PD_CS(A,AT,Atype,b,l,u,k,eps,maxit)
...
while 1==1 
    ...
    while 1==1 
        ...
        err = abs(obj-obj_old)/max(abs(obj),1);
        obj_old = obj;
        res = norm(x(:)-y(:),inf);       
        iter = iter + 1;       
        if err <=tol 

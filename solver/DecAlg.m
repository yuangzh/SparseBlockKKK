function [x,his] = DecAlg(x,A,b,k,workingset)
...
HandleObj = @(x)0.5*norm(A*x-b,'fro')^2;
n = size(A,2);
x = proj_l0(x,k);
...
%     B = [randperm(n,8)'; find_index_approximate_matrix(x,Q,p,8)];
    B = [randperm(n,workingset(1))'; find_index_approximate_matrix(x,Q,p,workingset(2))];
...
%       end
    ...
    if(mean(changes)<1e-5),break;end
end
% plot(his)
fprintf('\n');
...
function [index] = find_index_approximate_matrix(X,Q,P,num)
...
function [x_best] = quad_l0c_global(Q,p,k)
